...
## V2.2+ 扩展（Q-8 多平台发布自动化，半自动·不代发）

- `publish_automation.py`（**半自动·不真实调用任何平台 API**）：在 `render_variants.build_package`
  之上批量为多平台生成：各平台发布包 + 统一发布元数据（主画幅/AIGC 声明状态）+
  一键发布清单 `09_publish/publish-checklist.md`（逐平台勾选）+ 汇总
  `09_publish/publish-automation.yaml`（机读，供 D-7 观众反馈闭环关联）。
  发布为不可逆动作，须经 G9 导演终确认后人工逐平台发布，脚本不代发。
